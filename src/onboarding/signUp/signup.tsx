...
const Login: React.FC = () => {
    ...
    const handleSignupData = () => {
        ...
        if (password.trim() === "") {
            alert("Password should be atleast 8 letter long");
            return;
        }
        ...
        <div className="min-h-screen flex justify-center items-center md:bg-[#F0FDF4]">
            <div className="md:w-1/2 md:rounded-xl md:bg-[#FFFFFF]">
                <div className="flex flex-col items-center justify-center text-center py-10 md:m-auto lg:w-2/3 xl:w-1/2">
                    <h1 className="text-3xl text-green-600 font-bold mb-3">AgriHealth</h1>
                    <p className="mb-6 text-gray-500 text-lg font-medium">Create Your Account</p>
                    <div className="p-4 bg-white">

                        {/* Fullname section */}
                        <div className={`relative w-full flex border border-gray-300 rounded-md mb-7 ${isNameFocused || fullName ? "border-green-600" : ""}`}>
                            <button className={`pl-2 ${isNameFocused || fullName ? "hidden" : "block"}`}>
                                <img src={cloudImages.profile} width={25} height={25} alt="email icon" />
                            </button>
                            {/* Label for Fullname */}
                            <label
                                className={`absolute left-8 top-2 text-gray-400 transition-all duration-200 ${isNameFocused || fullName
                        ...
                        {/* Password section */}
                        <div className={`relative w-full flex border border-gray-300 rounded-md mb-5 ${isPasswordFocused || password ? "border-green-600" : ""}`}>
